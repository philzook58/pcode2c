...
#ifdef __CPROVER__
#define pcode2c_printf(...) __CPROVER_printf("PCODE2C" __VA_ARGS__)
#else
#include <stdio.h>
#include <stdlib.h>
...
// Ghidra makes assumptions about PC being automatically updated (I think) that
// I am not covering
...
void init_CPUState(CPUState *state) {
  ...
  // memset(state->reg, 0, 0x2000);
  // memset(state->unique, 0, 0x200000);
  // memset(state->ram, 0, 0x200000);
}
...
void COPY(uint8_t *out_addr, size_t out_size, uint8_t *in_addr,
          size_t in_size) {
  pcode2c_printf("\tCOPY: %p %lu %p %lu\n", out_addr, out_size, in_addr,
                 in_size);
...
// TODO, once I separate out the address space from the offset, go back to
// function
/*
void LOAD(void *output_addr, size_t output_size, void *input0_addr, size_t
input0_size, void *input1_addr, size_t input1_size)
{
    void *temp;
    pcode2c_printf("LOAD: %p %lu %p %lu %p %lu\n", output_addr, output_size,
input0_addr, input0_size, input1_addr, input1_size); memmove(&temp, input1_addr,
input1_size); pcode2c_printf("fllksjdklfjklsdfklskfjloo"); fflush(0);
    memmove(output_addr, temp, output_size);
} */

// Hack.
#define LOAD(output_addr, output_size, input0_addr, input0_size, input1_addr,  \
             input1_size)                                                      \
  {                                                                            \
    size_t temp;                                                               \
    pcode2c_printf("\tLOAD: %p %lu %p %lu %p %lu\n", output_addr, output_size, \
                   input0_addr, input0_size, input1_addr, input1_size);        \
    memmove(&temp, input1_addr, input1_size);                                  \
    pcode2c_printf("\tload address: %lu\n", temp);                             \
    fflush(0);                                                                 \
 ...
 */
#define STORE(input0_addr, input0_size, input1_addr, input1_size, input2_addr, \
              input2_size)                                                     \
  {                                                                            \
    size_t temp;                                                               \
    pcode2c_printf("\tSTORE: %p %lu %p %lu %p %lu\n", input0_addr,             \
                   input0_size, input1_addr, input1_size, input2_addr,         \
                   input2_size);                                               \
    memmove(&temp, input1_addr, input1_size);                                  \
...
#define CBRANCH_GOTO_TEMPFIX(pc_space, pc_addr, pc_size, cond_space,           \
                             cond_offset, cond_size)                           \
  {                                                                            \
    pcode2c_printf("\tCBRANCH_GOTO_TEMPFIX: %lu %lu %s %lu\n", cond_offset,    \
                   cond_size, #pc_addr, pc_size);                              \
    uint8_t cond;                                                              \
    assert(pc_size == 8);                                                      \
    assert(cond_size == 1);                                                    \
...
void SUBPIECE(uint8_t *out_addr, size_t out_size, uint8_t *in_addr,
              size_t in_size, uint8_t *input1_addr, size_t input1_size) {
  size_t offset;
  memmove(&offset, input1_addr, input1_size);
  size_t cpy_size =
      in_size - offset > out_size ? out_size : in_size - offset; // min
  memmove(out_addr, in_addr + offset, cpy_size);
}

// TODO: We can have something smaller. Unroll some kind of bit twiddling
// popcount.
...
void POPCOUNT(uint8_t *out_addr, size_t out_size, uint8_t *in_addr,
              size_t in_size) {
...
/*
void INT_EQUAL(uint8_t *out_addr, size_t out_size, uint8_t *in1_addr,
               size_t in1_size, uint8_t *in2_addr, size_t in2_size) {
  pcode2c_printf("\tINT_EQUAL: %p %lu %p %lu %p %lu\n", out_addr, out_size,
                 in1_addr, in1_size, in2_addr, in2_size);
  ...
  // out = out == 0 ? 1 : 0;
  out = in1_addr[0] == in2_addr[0] && in1_addr[1] == in2_addr[1] &&
                in1_addr[2] == in2_addr[2] && in1_addr[3] == in2_addr[3] &&
                in1_addr[4] == in2_addr[4] && in1_addr[5] == in2_addr[5] &&
                in1_addr[6] == in2_addr[6] && in1_addr[7] == in2_addr[7]
            ? 1
            : 0;
  memmove(out_addr, &out, 1);
}

void INT_NOTEQUAL(uint8_t *out_addr, size_t out_size, uint8_t *in1_addr,
                  size_t in1_size, uint8_t *in2_addr, size_t in2_size) {
...
#define IPRED(name, opexpr, sign)                                              \
  void name(uint8_t *out_addr, size_t out_size, uint8_t *in0_addr,             \
            size_t in0_size, uint8_t *in1_addr, size_t in1_size) {             \
    pcode2c_printf("\t" #name ": %p %lu %p %lu %p %lu\n", out_addr, out_size,  \
                   in0_addr, in0_size, in1_addr, in1_size);                    \
    ...
    if (in0_size == 1) {                                                       \
      ...
      assert(in0_size == 1 || in0_size == 2 || in0_size == 4 ||                \
             in0_size == 8);                                                   \
    }                                                                          \
  }

IPRED(INT_EQUAL, in0 == in1, u)
IPRED(INT_NOTEQUAL, in0 != in1, u)
IPRED(INT_LESS, in0 < in1, u)
IPRED(INT_SLESS, in0 < in1, )
IPRED(INT_LESS_EQUAL, in0 <= in1, u)
IPRED(INT_SLESS_EQUAL, in0 < in1, )

void INT_ZEXT(uint8_t *out_addr, size_t out_size, uint8_t *in1_addr,
              size_t in1_size) {
  pcode2c_printf("\tINT_ZEXT: %p %lu %p %lu\n", out_addr, out_size, in1_addr,
                 in1_size);
...
void INT_SEXT(uint8_t *out_addr, size_t out_size, uint8_t *in1_addr,
              size_t in1_size) {
  ...
  // x = x & ((1U << b) - 1); // (Skip this if bits in x above position b are
  // already zero.)
  r = (x ^ m) - m;
...
#define IBINOP_PRECOND(name, op, precond)                                      \
  void name(uint8_t *out_addr, size_t out_size, uint8_t *in1_addr,             \
            size_t in1_size, uint8_t *in2_addr, size_t in2_size) {             \
    pcode2c_printf("\t" #name ": %p %lu %p %lu %p %lu\n", (void *)out_addr,    \
                   out_size, in1_addr, in1_size, in2_addr, in2_size);          \
    assert(precond);                                                           \
    if (out_size == 1) {                                                       \
      int8_t in1, in2, out;                                                    \
      ...
      assert(out_size == 1 || out_size == 2 || out_size == 4 ||                \
             out_size == 8);                                                   \
    }                                                                          \
  }

#define IBINOP(name, op)                                                       \
  IBINOP_PRECOND(name, op, out_size == in1_size && out_size == in2_size)
IBINOP(INT_ADD, +)
IBINOP(INT_SUB, -)
...
void BOOL_NEGATE(uint8_t *output_addr, size_t output_size, uint8_t *input_addr,
                 size_t input_size) {
...
#define BOOL_BINOP(name, op)                                                   \
  void name(uint8_t *output_addr, size_t output_size, uint8_t *input0_addr,    \
            size_t input0_size, uint8_t *input1_addr, size_t input1_size) {    \
    pcode2c_printf("\t" #name ": %p %lu %p %lu %p %lu\n", output_addr,         \
                   output_size, input0_addr, input0_size, input1_addr,         \
                   input1_size);                                               \
...
#define BINOP(name, op, size, type) \
    void name(void *out_addr, size_t out_size, void *in1_addr, size_t in1_size,
void *in2_addr, size_t in2_size) \
    { \
        type in1, in2, out; \
...
#define FBINOP(name, op) BINOP(name##32, op, 32, float) BINOP(name##64, op, 64,
double)
